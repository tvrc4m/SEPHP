...
class UploadFilePlugin extends Plugin {
    ...
    public function run($data){
        ...
        $fileinfo=$this->upload($data['savepath']);
    ...
    private $config =   array(
        ...
        'imageClassPath'    =>  'Image.plugin.php',   // 图库类包路径
    ...
    private function save($file) {
        ...
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
                ...
                // 生成图像缩略图
                include_once(PLUGIN.$this->imageClassPath);
    ...
    private function getSaveName($filename) {
        ...
        if(!empty($this->config['savename'])){

            if(empty($rule))

                return $this->config['savename'].'.'.$filename['extension'];

            return md5($this->config['savename']).'.'.$filename['extension'];
        }
